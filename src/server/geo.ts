/** Distances and travel times, all measured from the Durst HQ in Brixen. */
...
export function deriveTravel(input: {
  ...
  const bearing = hasCoords ? bearingDeg(HQ.lat, HQ.lon, input.lat!, input.lon!) : null;
  ...
  if (distanceKm === null) {
    distanceKm = hasCoords
      ? roadDistanceKm(haversineKm(HQ.lat, HQ.lon, input.lat!, input.lon!))
      : 0;
  }
